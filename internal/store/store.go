...
func (s *Store) searchTasks(ctx context.Context, q, lane string, limit int) ([]SearchResult, error) {
	...
	if comments {
		parts = append(parts, `SELECT 'tasks' scope,t.id::text key,t.lane session,t.kind,t.title,`+
			`COALESCE(NULLIF(ts_headline('simple',c.body,plainto_tsquery('simple',$2),'MaxWords=24,MinWords=8'),''),left(c.body,160)) snippet,`+
			refsExpr+` refs,t.created_at,1 match_rank FROM task_comments c JOIN tasks t ON t.id=c.task_id WHERE ($1='' OR t.lane=$1) AND (to_tsvector('simple',c.body) @@ plainto_tsquery('simple',$2) OR c.body ILIKE '%'||$4||'%' ESCAPE '\') AND NOT `+titleMatch)
	}
